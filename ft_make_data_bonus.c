...
/*   Updated: 2020/12/07 09:10:41 by kde-rooi      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static t_flag	*data_hex(t_flag *flag, va_list args)
{
	...
	if (flag->type == 'x')
		flag->data = (flag->hash) ? ft_itoa_hex(nbr, flag, 0) :
		ft_itoa_base(nbr, "0123456789abcdef");
	if (flag->type == 'X')
		flag->data = (flag->hash) ? ft_itoa_hex(nbr, flag, 1) :
		ft_itoa_base(nbr, "0123456789ABCDEF");
	return (flag);
}
